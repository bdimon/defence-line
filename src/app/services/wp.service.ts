...
export class WpService {
  ...
  loadCategories(): Observable<any[]> {
    ...
    return this.http.get<any[]>(`${this.API_URL}/categories`).pipe(
      map((cats: any[]) => {
  ...
  getPost(id: number): Observable<any> {
    return this.http.get(`${this.API_URL}/posts/${id}?_embed`);
  }

  getComments(postId: number, page: number = 1): Observable<any[]> {
  const params = new HttpParams()
    .set('_embed', '')
    .set('page', page.toString())
    .set('post', postId.toString());
  return this.http.get<any[]>(`${this.API_URL}/comments`, { params });
}
}